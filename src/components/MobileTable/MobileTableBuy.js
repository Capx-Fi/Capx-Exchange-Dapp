...
function MobileTableBuy({
	...
	return (
		<>
			<div
				className={`mobileTableBuy h-fit-content max-h-60v w-85v overflow-y-hidden`}
			>
				{isInfo ? (
				...
				<div
					className={`_body border border-dark-50 rounded-br-md rounded-bl-md h-fit-content max-h-55v overflow-y-auto ${
						isInfo ? "phone:mb-10" : ""
					}`}
				>
					{loading ? (
		...
		>
			<div className="_leftContainer text-left text-white">
				<p
					...
					className="_assetName font-bold uppercase tracking-wider bg-dark-300 p-2 rounded-lg px-3 w-fit-content max-w-30 border border-dark-50 text-caption-2 text-center"
				>
					{token.asset}
				</p>
